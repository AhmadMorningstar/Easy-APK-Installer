...
import webbrowser # Added for opening the YouTube link
...
def save_path_to_registry(path):
    """
    Saves the given folder path to the Windows Registry.
    """
    try:
        # Create or open the registry key
        key = winreg.CreateKey(winreg.HKEY_CURRENT_USER, REGISTRY_PATH)
        # Set the 'LastFolderPath' value
...
def print_menu():
    ...
    print(f"{Fore.GREEN}7. YouTube Guide") # Added new menu option
    print(f"{Fore.GREEN}8. Exit{Style.RESET_ALL}") # Exit button is now 8
...
def open_youtube_guide():
    """
    Opens the YouTube guide link in the default web browser and immediately returns to the menu.
    """
    ...
    # The script now immediately returns to the main loop without waiting for user input.
...
def set_folder_path_menu():
    """
    Handles the folder path selection menu and saves the path to the registry.
    """
    ...
    if os.path.isdir(path):
        ...
        print(f"{Fore.GREEN}Folder path set to: '{path}' and saved to registry.{Style.RESET_ALL}")
    else:
...
def install_apks_for_device(device_serial, folder_path, apks_to_install, allow_skip=True, grant_permissions=False):
    ...
    Args:
        device_serial (str): The serial number of the ADB device.
        folder_path (str): The path to the folder containing the APKs.
        apks_to_install (list): A list of APK filenames to install.
        allow_skip (bool): If True, allows the user to press 's' to skip an installation.
        grant_permissions (bool): If True, use the '-g' flag to auto-grant all permissions.
    """
    success_count = 0
    failure_count = 0
    skipped_count = 0
    ...
    for i, apk_file in enumerate(apks_to_install):
        ...
        # Build the adb command with conditional flags
        command = ['adb', '-s', device_serial, 'install', '-r']
        if grant_permissions:
            command.append('-g')
        command.append(apk_full_path)
        
        # Start the adb install process without capturing stdout/stderr immediately
        ...
        while process.poll() is None:
            ...
            if allow_skip:
                # Check for a keyboard press. This is a very fast, non-blocking check.
                if msvcrt.kbhit():
                    ...
                    if key == 's':
                        ...
                        process.terminate()  # Use terminate for a clean shutdown
                        skipped_this_apk = True
                        break
            time.sleep(0.1) # Small sleep to control spinner speed
        
        sys.stdout.write("\r" + " " * 80 + "\r") # Clear the final spinner line
        sys.stdout.flush()
        
        # Now that the process is done (either by finishing or being killed), get the output
        stdout, stderr = process.communicate()
        
        if skipped_this_apk:
            skipped_count += 1
            continue  # Move to the next APK in the loop
            
        # Check return code and output for success
    ...
    while True:
        try:
            ...
            if choice == 0:
                ...
                install_apks_for_device(SELECTED_DEVICE['serial'], APK_FOLDER_PATH, [selected_apk_file], allow_skip=True, grant_permissions=grant_permissions)
                break
            else:
...
if __name__ == "__main__":
    ...
    if not devices_on_startup:
        ...
        print(f"Press any key to continue to the main menu...{Style.RESET_ALL}")
        msvcrt.getch()

    # Load last used path and scan for APKs
    APK_FOLDER_PATH = load_path_from_registry()
    scan_for_apks()

    while True:
        print_menu()
        # Use input() to allow the user to see their selection, which is a better UX
        choice = input()
        
        # We now check the raw input string instead of the non-echoed getch()
        if choice == '1':
            select_device_menu()
        elif choice == '2':
            ...
            show_about_menu()
        elif choice == '7': # New choice for YouTube link
            open_youtube_guide()
        elif choice == '8': # Exit is now option 8
